<template>
	<div class='container containerHeight' @touchstart="touchStart($event)" @touchmove="touchMove($event)" @touchend="touchEnd($event)">
		<div class='contents'>
			<div class='head'>
				<div class='head-left'>
				...
				<div v-for='(item,index) in  list' :key='index'>
					<div class='pad-bot showdaw-color'>
						<div class='new-tit'>
							<div class='new-head'>
								...
								<div class='tit'>
									...
									<span v-if='item.bizType == 3'>家政清洁</span>
									<span v-if='item.bizType == 4'>窗帘清洁</span>
									<span v-if='item.bizType == 5'>地毯清洁</span>
									<span v-if='item.bizType == 6'>酒店客房清洁</span>
									<span v-if='item.bizType == 7'>水晶灯清洗</span>
									<span v-if='item.bizType == 8'>床垫清洗</span>
									<span v-if='item.bizType == 9'>石材养护</span>
									<span v-if='item.bizType == 10'>空调清洗</span>
									<span v-if='item.bizType == 11'>商城</span>
								</div>
						...
						<div class='remind'>
							...
							<div class='remind-info'>{{item.content}}</div>
						</div>
...
<script>
	...
	export default {
		...
		methods: {
			...mapActions(['addShowDes', 'addShowSignId', 'addDesCont','collageServerUnread']),
			collage(e) {
			...
			toDeatail(e, index) {
				...
				if (e.isRead == 2) {
					...
					axios.post('/business/t-business-notice-employee/updateReadById', params).then(res => {
						if (res.meta.success) {
							// this.badgeNumber()
							let num = this.$store.state.serverUnread +1
							this.collageServerUnread(num)
			...
			getList() {
				...
				axios.post(
					'/business/t-business-notice-employee/getEmployeeNoticeByEmployeeId',
					formdata
				).then(res => {
					if (res.meta.success) {
			...
			badgeNumber() {
				...
				// axios.post('/business/t-business-notice-user/unReadNum', {}).then(res => {
				// 				              if(res.meta.success){
				// 								  if(res.data==0){
