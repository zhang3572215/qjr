	...
	<div>
		...
		<div class='content-upload'>
			<div class="upload-tit">
				签到图片<span class='unnecessary'>(非必传)</span>
			</div>
		...
		<div class='positionInfo'>
			...
			<span v-if='showRange'>已经进入签到范围{{addrName}} </span>
			<span class='discontent' v-else>您当前未在签到范围内</span>
		...
		<showSuccess v-if='isSuccess' :successInfo='"签到成功"'></showSuccess>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			signConfirm() {
				...
				this.isLoad = true
				axios.post('/office/t-office-compose/signIn', params).then(res => {
					this.isLoad = false
					if (res.meta.success) {
						this.isSuccess = true
