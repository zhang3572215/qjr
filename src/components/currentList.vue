<template>
	...
	<div class='containerHeight contents' >
		<!-- <dates v-if='showVue' :msg="dateFlag" @biography='collageDate'></dates> -->
		<div>
			<head-Piece :headLeft='"true"'  :hiddenLeft='"true"' v-show="!typeFlag">
				<template slot="content">当前订单</template>
			</head-Piece>
			<!-- <div class='head'>
			...
			<div class='content-btn' :class="typeFlag?'collageTop':''">
				<div class='btn-info'>
					<div class='btn' v-for='(item,index) in typeList ' :key='index' @click="collageTitle(item.types)" :class='types == item.types?"sign-color":""'>
						<div class='btn-num' v-if='item.num!=0'>
							{{item.num}}
						</div>
						<div>
							{{item.title}}
					...
					<div class='list' @click="gotoDetail(item.id,item.buyType)" v-for='(item,index) in list' >
							...
							<div class='list-head'>
								...
								<div class='head-right'>
									<span v-if='item.orderStatus==1'>
                                         <span   v-if='item.payType==1'>待支付</span>
                                        <span v-if='item.payType==2'>对公转账</span>
                                         <span v-if='item.payType==3'>月结</span>
                                    </span>
									<span v-if='item.orderStatus==2'>
                                       <span   v-if='item.payType==1'>已支付</span>
                                       <span v-if='item.payType==2'>对公转账</span>
                                        <span v-if='item.payType==3'>月结</span>
                                    </span>
       				...
       				<span v-if='item.orderStatus==6'>关闭</span> -->
							...
							<div class='list-btn' v-if='item.orderStatus==1 || (item.buyType==1 && item.orderStatus==2) || (item.buyType==1 && item.orderStatus==3)   || item.orderStatus==3 ||item.orderStatus==5 ||(item.orderStatus==2 && item.payType==3 && item.isSure ==1)'>
								<button class='btn-pay'v-if='item.orderStatus==1  && item.payType==1' @click.stop.prevent='pay(item.id,item.payType,item.payableMoney,item.isSure)'> 支付</button>
                                 <button class='btn-pay' v-if='item.orderStatus==1 &&  item.payType==2 && item.isSure ==2' @click.stop.prevent='pay(item.id,item.payType,item.payableMoney,item.isSure)'>确认转账</button>
                                <button class='btn-pay' v-if='item.orderStatus==1 &&  item.payType==2 && item.isSure ==1' @click.stop.prevent='pay(item.id,item.payType,item.payableMoney,item.isSure)'>修改凭证</button>
                                 <button class='btn-pay' v-if='item.orderStatus==1 &&  item.payType==3 && item.isSure ==2' @click.stop.prevent='pay(item.id,item.payType,item.payableMoney,item.isSure)'>确认签约</button>
                                 <button class='serveDe' v-if='item.payStatus==1' @click.stop.prevent="toServeList(item.id,item.productType)">服务详情</button>
                                  <button class='btn-pay' v-if='(item.orderStatus==1 || item.orderStatus==2 || item.orderStatus==3)  &&  item.payType==3 && item.isSure ==1' @click.stop.prevent='pay(item.id,item.payType,item.payableMoney,item.isSure)'>月结</button>
								<button class='btn-cancel' v-if='item.orderStatus==1 && item.payType!=3' @click.stop.prevent="cancle(item.id)">取消订单</button>
                                <button class='btn-cancel' v-if='item.orderStatus==1 && item.payType==3 &&item.isSure==2' @click.stop.prevent="cancle(item.id)">取消订单</button>
						...
						<div v-if='types==4'>
							...
							<div class='list-center clearfloat'>
								<div class='center-left'>
									<img class='left-img' src='../assets/carpetIcon.png'></img>
								...
								<div class='center-tit'>
									<div class='content-info'>
										...
										<span v-if='item.frequency==7'>7次/周</span> -->
											<!-- <span>({{item.weeks}})</span> -->
											<span>{{item.className}}{{item.classPriceName}}</span>
										</div>
										<!-- <div class='center-week'>
										{{item.weeks}}
									</div> -->
						...
						<div v-if='types==3 ||types ==5 || types==6'>
							<div class='list-head'>
								<div class='head-left'>
									<img src='../assets/time.png'></img>
									<div class='left-tit'>下单时间:<span>{{item.orderTime}}</span></div>
						...
						<span v-if='item.orderStatus==6'>关闭</span> -->
							...
							<div class='list-center clearfloat'>
								<div class='center-left'>
									<img class='left-img' v-if='types==5' src='../assets/curtainIcon.png'></img>
									<img class='left-img' v-if='types==3' src='../assets/airIcon.png'></img>
									<img class='left-img' v-if='types==6' src='../assets/mattressIcon.png'></img>
								</div>
								<div class='center-tit'>
									<div class='content-info'>
										<div class="tit-info">
											<span v-if='types==3'>空调清洗</span>
											<span v-if='types==5'>窗帘清洗</span>
											<span v-if='types==6'>床垫清洗</span>
											<span class='tit-right'>
												￥{{item.price}}
											</span>
										</div>
						...
						<div v-if='item.productType==111 && (item.orderStatus==1 ||item.orderStatus==2 || item.orderStatus==3)'>
							<div class='list-head'>
								<div class='head-left'>
									<img src='../assets/time.png'></img>
									<div class='left-tit'>下单时间:<span>{{item.orderTime}}</span></div>
...
<script type="text/javascript" src="https://js.cdn.aliyun.dcloud.net.cn/dev/uni-app/uni.webdiv.1.5.2.js"></script>
		...
		methods: {
			...
			confirm() { //确定取消订单
				...
				if (this.type == 1) {
					...
					var url = ''
					if (this.types == 2) { //办公清洁取消订单
						url = '/office/t-office-order/cancelOrder'
					} else if (this.types == 4) { //地毯取消订单
						url = '/carpet/t-carpet-order/cancelOrder'
					} else if (this.types == 3 || this.types == 5 || this.types == 6) { //空调，窗帘，床垫清洗取消订单
						url = '/technology/t-technology-order/cancelOrder'
					} else if (this.types == 7) {
						url = '/shop/t-shop-order/cancelOrder'
					}
			...
			gotoDetail(id, status) {
				...
				} else if (this.types == 3 || this.types == 5 || this.types == 6) {
					let info = {
						orderId: id
					}
					this.$router.push({
				...
				if (this.types == 2) {
                    ...
                    if(payType==1){
                      ...
                      this.$router.push({
                        ...
                        let info={}
                        if(isSure==1){
                               info= {'id':id,'payableMoney':payableMoney,payFlag:1,showBtn:true}
                        }else{
                               info= {'id':id,'payableMoney':payableMoney,payFlag:1,showBtn:false}
                        }
                       
                        this.$router.push({'name':"transferAccounts",params:info})
                    }else{
                        if(isSure==2){
                             let info= {'id':id,'payableMoney':payableMoney}
			...
			getList(e) {
				...
				var url = ''
				if (this.types == 2) {
					url = '/office/t-office-order/getCurrentOfficeOrderList'
				} else if (this.types == 4) {
					url = '/carpet/t-carpet-order/getCurrentCarpetOrderList'
				} else if (this.types == 3 || this.types == 5 || this.types == 6) {
					url = '/technology/t-technology-order/getCurrentTecOrderList'
					var types = ''
					switch (this.types) {
						case 3:
							types = 10;
					...
					formdata.append('productType', types)
				} else if (this.types == 7) {
					url = '/shop/t-shop-order/getShopOrderList'
				}
				axios.post(
					url,
					formdata
			...
			getAllCount() {
				axios.post('/office/t-office-order/getCurrentOrderCount').then(res => {
					if (res.meta.success) {
						let data = res.data
						this.typeList.map(item => {
							if (item.types == 2) {
								item.num = data.currentOfficeOrderCount
							} else if (item.types == 3) {
								item.num = data.currentAirOrderCount
							} else if (item.types == 4) {
								item.num = data.currentCarpetOrderCount
							} else if (item.types == 5) {
								item.num = data.currentCurtianOrderCount
							} else if (item.types == 6) {
								item.num = data.currentMattressOrderCount
							} else if (item.types == 7) {
								item.num = data.currentShopOrderCount
							}
						})
					}
				})
