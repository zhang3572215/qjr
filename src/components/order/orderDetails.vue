	...
	<div class='content'>
		...
		<div class='order-start' v-if='showContent' :class='showHead?"collage-head":""'>
			<div class='start' v-if='info.orderStatus==1'>
				<div v-if='info.payType ==1'>待支付</div>
                <!-- <div v-if='info.payType==1'>-在线支付</div> -->
                <div  v-if='info.payType==2'>对公转账<div v-if='info.isSure==2'>-待客户确认转帐</div><div v-if='info.isSure==1'>-客户确认转帐-待财务确认</div></div>
                <div  v-if='info.payType==3'>月结<div v-if='info.isSure==2'>-待确认签约</div><div v-if='info.isSure==1'>-已确认签约</div></div>
			</div>
			<div class='start' v-if='info.orderStatus==2'>
                <div v-if='info.payType ==1'> 已支付</div>
                 <div  v-if='info.payType==2'>对公转账</div>
                <div  v-if='info.payType==3'>月结<div v-if='info.isSure==1'>-已确认签约</div></div>
			</div>
			<div class='start' v-if='info.orderStatus==3'>
			    服务中
              <!-- <div v-if='info.payType !=2 && info.payType!=3'>服务中</div> -->
              <!-- <div  v-if='info.payType==3'>月结<div v-if='info.isSure==1'>-已确认签约</div></div> -->
			</div>
           ...
           <!--     <div v-if='info.payType==1'>-在线支付</div>
                <div  v-if='info.payType==2'>-对公转账</div>
                 <div  v-if='info.payType==3'>-月结</div> -->
			</div>
		...
		<div v-if='info.productType==2'>
			...
			<div class='meal' v-if='showContent'>
				...
				<div class='meal-bottom' v-if='showContent'>
					...
					<div class='stages' v-if='info.isPeriod==1'>
						<div></div>
                        	<div class='moneys'>(分为{{info.periodNum}}个月,<span>已结算{{info.payPeriod}}月</span>)</div>
			...
			<div class='order-bottom' v-if='(showContent  && info.orderStatus==3) ||(showContent  && info.orderStatus==1)  ||  info.payStatus==1 '>
				...
				<div @click="pay" v-if='info.orderStatus==1 && info.payType==1'>去支付</div>
                <div @click="pay" v-if='info.orderStatus==1 && info.payType==2 && info.isSure ==1'>修改凭证</div>
                      <div @click="pay" v-if='info.orderStatus==1 &&  info.payType==2 && info.isSure ==2'>确认转账</div>
                <div @click="pay" v-if='info.orderStatus==1 &&  info.payType==3 && info.isSure ==2'>确认签约</div>
                 <div @click="pay" v-if='(info.orderStatus==1 || info.orderStatus==2 || info.orderStatus==3) &&  info.payType==3 && info.isSure ==1'>月结</div>
				<div class='complaint' v-if='info.orderStatus==1 && info.payType!=3' @click="cancle(info.id)">取消订单</div>
                <div class='complaint' v-if='info.orderStatus==1 && info.payType==3  &&info.isSure==2' @click="cancle(info.id)">取消订单</div>
		...
		<div v-if='info.productType==4'>
			...
			<div class='meal' v-if='showContent'>
				<div class='meal-top'>
					...
					<div>
						...
						<div class='remain'>
							开始服务日期：<span>{{info.startDate}}</span>
						</div>
			...
			<div class='order-info' v-if='showContent'>
				<div class='order-num'>
					<div>订单编号：<span>{{info.orderNo}}</span></div>
					<div class='info-btn' v-clipboard:copy="copyInfo" v-clipboard:success="copy">
						复制
			...
			<div class='order-bottom' v-if='(showContent  && info.orderStatus==3) ||(showContent  && info.orderStatus==1)  '>
				<!-- <div class='appointment' v-if='info.orderStatus==2'>预约</div> -->
				<!-- <div v-if='info.orderStatus==3'>联系清洁师</div> -->
				<!-- <div v-if='info.orderStatus==3' class='complaint' @click='comPlaint(info.id)'>发起投诉</div> -->
...
<script>
	...
	export default {
		...
		computed: {
			...mapGetters(['showCurrentListRefresh'])
		},
		methods: {
			...mapActions(['collageCltRefresh','addDesCont']),
			toCustomer() {
			...
			pay() {
                ...
                if(this.info.payType==1){
                  ...
                  this.$router.push({
                   ...
                   let info= {'id':this.info.id,'payableMoney':this.info.payableMoney,payFlag:1}
                  
                  this.$router.push({'name':"transferAccounts",params:info})
                }else{
                    
                    if(this.info.isSure==2){
                         let info= {'id':this.info.id,'payableMoney':this.info.payableMoney}
                         this.$router.push({'name':"monthlyBalance",params:info})
                    }else if(this.info.isSure==1){
                     let info= {'id':this.info.id}
		...
		// this.showHead = true
            ...
            this.isBack=this.$route.params.isBack
			this.getInfo()
