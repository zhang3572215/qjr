	...
	<div class='container'>
		...
		<div class='content'>
             <div class='content-step'>
                 <van-steps :active="active" active-color='#06b4fd'>
                   <van-step>提交订单</van-step>
                   <van-step>线上支付</van-step>
                   <van-step>安排服务</van-step>
                 </van-steps>
             </div>
				...
				<div class='order-info'>
					...
					<div class='info-new'>
						<div class='new-tit'>
							...
							<div class='money'>￥{{info.payableMoney}}</div>
						</div>
						<div v-if='info.buyType!=1'>
							<div class='order-times'>
								<!-- <span class='count mr-right'>{{info.frequency}}次/周</span>
								<span class='num mr-right'>({{info.totalNums }}次)</span>
								<span class='week  mr-right'>{{info.weeks}}</span> -->

								<!-- <span class='count mr-right'>清洁频率：<span>{{info.dailyFrequency}}</span>，<span>其中深度清洁：{{info.depthFrequency}}</span></span> -->
								<span class='count mr-right'><span>本次订单共有{{info.dailyNums+info.depthNums}}次服务</span></span>


								<!-- <span>(18-20点)</span> -->
							</div>
							<div class='start'>
								<!-- 开始日期：<span class='year'>{{info.startDate}}</span>	<span>({{info.beginTime}}-{{info.endTime}}点)</span> -->

								<!-- <span class='year'>清洁次数：<span>{{info.dailyNums+info.depthNums}}次</span>，<span>其中深度清洁：{{info.depthNums}}次</span></span> -->
								<span class='year'><span>其中深度清洁：{{info.depthNums}}次</span>，<span>日常清洁：{{info.dailyNums}}次</span></span>
							</div>
						</div>
						<!-- 	<div v-else>
							<div class='order-times'>
		...
		<div class='order-bottom'>
			<div class='actual'>
				<span>实付款：</span>
				<span class='money' v-if='info.periodMoney.length==0'>￥{{info.payableMoney}}</span>

				<span class='money' v-if='info.periodMoney.length!=0'>￥{{info.periodMoney}}</span>
			</div>
	...
	export default {
		...
		methods: {
			...
			payMent() {
				let that = this
				if (!this.showWx) {
					if (this.info.periodMoney.length == 0) {
						if (this.payType == 1) {
							let params = new FormData()
							params.append('orderId', this.id)
							params.append('payableMoney', this.info.payableMoney)
							axios.post('/finance/payment/createOfficeUnifiedOrderWx', params).then(res => {
								if (res.meta.success) {
						...
						if (this.payType == 2) {
							...
							params.append('orderId', this.id)
							params.append('payableMoney', this.info.payableMoney)
							axios.post('/finance/payment/createOfficeUnifiedOrderAli', params).then(res => {
								if (res.meta.success) {
						...
						if (this.payType == 1) {
							...
							console.log(555)
							console.log(this.info.periodMoney)
							let params = new FormData()
							params.append('orderId', this.id)
							params.append('payableMoney', this.info.periodMoney)
							axios.post('/finance/payment/createOfficeUnifiedOrderWx', params).then(res => {
								if (res.meta.success) {
						...
						if (this.payType == 2) {
							...
							axios.post('/finance/payment/createOfficeUnifiedOrderAli', params).then(res => {
								if (res.meta.success) {
									...
									// 	success(res) {
									// 		// 支付成功跳转页面
									// let id={'id':that.id,'type':that.info.buyType}
					...
					if (this.info.periodMoney.length == 0) {
						if (this.payType == 1) {					
							...
							params.append('orderId', this.id)
							params.append('payableMoney', this.info.payableMoney)
							axios.post('/finance/payment/createOfficeUnifiedOrderWxJsApi', params).then(res => {
								if (res.meta.success) {
...
<style>
    ...
    .content-step .van-step--finish .van-step__circle, .van-step--finish .van-step__line{
