<template>
	<div>
		...
		<div class='content' v-if='showContent'>
			<!-- <div class='order showdaw-color'>
				...
				<div class='order-info'>
					...
					<div class='info-new'>
						<div class='new-tit'>
							<div v-if='info.buyType==1'>办公室清洁-按次</div>
							<div v-if='info.buyType==2'>办公室清洁-月度</div>
							<div v-if='info.buyType==3'>办公室清洁-季度</div>
							<div v-if='info.buyType==4'>办公室清洁-年度</div>
							<div class='money'>￥{{info.orderMoney}}</div>
						</div>
						<div>
							<div class='order-times'>
								<span class='count'>3次/周</span>
								<span class='num'>(12次)</span>
								<span class='week'>周一、周三、周五</span>
								<span>(18-20点)</span>
							</div>
							<div class='start'>
								开始日期：<span class='year'>{{info.startDate}}</span><span>09:30:00</span>
							</div>
						</div>
					</div>
		...
		<div class='order-bottom' v-if='showContent'>
			<div class='actual'>
				<span>实付款：</span>
				<span class='money'>￥{{info.periodMoney}}</span>
			</div>
			...
			payMent() {
				...
				if (!this.showWx) {
					if (this.payType == 1) {
						...
						let params = new FormData()
						params.append('periodId', this.periodId)
						params.append('periodMoney', this.info.periodMoney)
						axios.post('/finance/payment/createOfficePeriodUnifiedOrderWx', params).then(res => {
							if (res.meta.success) {
								...
								// 		// 支付成功跳转页面
								// 		uni.navigateTo({
								// 			url: '/pages/home/success?id='+that.info.id+'&type='+that.info.buyType
								// 		});
								// 	},
								// 	fail(err) {
								// 		// console.log(err)
					...
					if (this.payType == 2) {
						...
						params.append('periodId', this.periodId)
						params.append('periodMoney', this.info.periodMoney)
						axios.post('/finance/payment/createOfficePeriodUnifiedOrderAli', params).then(res => {
							if (res.meta.success) {
								// console.log(res.data)
					...
					params.append('periodId', this.periodId)
					params.append('periodMoney', this.info.periodMoney)
					axios.post('/finance/payment/createOfficePeriodUnifiedOrderWxJs', params).then(res => {
						if (res.meta.success) {
