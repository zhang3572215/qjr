<template>
	...
	<div class='conntent-info'  >
		...
		<div class='order'>
			...
			<div class='order-head'>
				...
				<div class='order-tit'>
					<div class='order-set'>
						<div>
							<span v-if='classification==10'>
						     	{{goodinfo}}
							</span>
							<span v-else-if='classification==8'>
							 	{{goodinfo}}
							</span>
							<span v-else>
								{{goodinfo}}
							 	<!-- 纱帘3*4(±1)米1条+纱帘3*4(±1)米1条+纱帘3*4(±1)米1条+纱帘3*4(±1)米1条 -->
							</span>
					...
					<div class='order-times'>
						<span class='count' v-if='type==2'>拼团价￥{{assemblePrice}}</span>
						<span class='count' v-else>原价￥{{originalPrice}}</span>
						<span class='num'  v-if='type==2'>原价￥{{originalPrice}}</span>
		...
		<!-- 	<div class='order-coupon' @click="purposeClose">
			...
			<div class='exhort'>
				<div class='exhort-tit'>特殊需求备注</div>
				 <textarea class='exhort-input' placeholder="请在这里写下您的特殊需求吧!" v-model="remark" @blur="resetHeight" @focus="toHeight"/>
				</div>
		<div class='explain'>
		...
		<div class='order-bottom'>
			<div class='tab-left'>
				<div class='stay' v-if='type==2'>待支付：￥{{assemblePrice}}</div>
				<div class='stay' v-else>待支付：￥{{originalPrice}}</div>
		...
		<div class='time-info' :class='collageTime?"showTime-info":""'>
			<div class='elastic-head'>
				<div class='time-list'>
					<div v-for='(item,index)  in dateList ' :key='index' v-if='index!=0' :class=' collageDate==item.week ?"showTimeColor":""' @click="tabDate(item.week,item.date,index)">
		...
		methods:{
		   ...
		   resetHeight(){
			...
			confirm(){
				...
				// }
					...
					if(this.type==1){
						...
						params.append('orderMoney',this.assemblePrice)
						params.append('price',this.assemblePrice)
						params.append('remark',this.remark)
						params.append('serviceDate',this.selectDateInfo)
						params.append('specId',this.specId)
						params.append('addressId',this.addressId)
					...
					axios.post('/technology/t-technology-order/postTechnologyOrder',
					params).then(res => {
						if(res.meta.success){
							console.log(res)
								this.isLoade=false
			...
			postAssemble(){
				let params=new FormData()
				params.append('productId',this.productId)
				axios.post('/business/t-business-order-assemble/postTecAssemble',params).then(res=>{
					 if(res.meta.success){
	  ...
	  this.ergodicDate()
			...
			this.list = this.$route.params
			 ...
			 this.type=this.list.type
			  this.classification=this.list.classification
