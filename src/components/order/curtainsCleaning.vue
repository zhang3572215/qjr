<template>
	<div>
		...
		<head-Piece>
			<template slot="content">窗帘清洁</template>
		</head-Piece>
	   ...
	   <div class='order'>
	   		    ...
	   		    <div class='elastic-select'>
	   		    	<div class='select-title'>预约服务时辰</div>
	   ...
	   <!-- 		<div class='explain'>
	                 ...
	                 <image v-if='flag' src='../../static/true.png'></image>
	   			  </div>
	   			  <div  class='explain-info' @click="toBuyText">
	   				  购买视为同意 <text  class='explain-info-tit'>《清洁人用户购买协议》、《办公室（2020）版"服务说明"》</text>
	   			  </div>
	   		</div> -->
	   </div>
		...
		methods:{
		   ...
		   resetHeight(){
			...
			getCoupon() {
				...
				axios.post('/qjrapi/business/t-business-coupon/getEnableCoupons',{productType:2,buyType:this.id}).then(res => {
			...
			getInfo() {
				axios.post('/qjrapi/product/t-product-office/getProductOffice', {
					id: this.id
				}).then(res => {
					console.log(res)
					if (res.meta.success) {
			...
			receive(){
				...
				axios.post('/office/t-office-order/createChuangLianOrderByExperience',params).then(res => {
					if (res.meta.success) {	
					   this.isLoade=false
					   this.showSuc=true
					   setTimeout(()=>{
