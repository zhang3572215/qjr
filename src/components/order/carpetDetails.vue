				...
				<div class='meal-top'>
					...
					<div>
						...
						<div class='meal-times'>
							...
							<span class='num'>购买规格：{{info.className}}{{info.classPriceName}}</span>
                        ...
                        <div class='remain'>
                        	预约取件时间：<span>{{info.serviceDate}} </span>{{info.beginTime}}
                        </div>
						<!-- <div class='remain'>
	...
	export default {
		...
		methods: {
			...
			confirms() {
				...
				let params = new FormData()
				params.append('orderId', this.id)
				axios.post('/carpet/t-carpet-order/cancelOrder', params).then(res => {
					if (res.meta.success) {
