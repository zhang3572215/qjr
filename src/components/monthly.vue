				...
				<div class='content-img' v-if='banner.length !=0'>
					<div v-if="banner.length > 1">
						<swiper :options="swiperOptionAd" ref='silderAd' class="swiper-container">
							...
							<!-- 常见的小圆点 -->
							<div class="swiper-pagination2" v-if="banner.length > 0 " v-for="(item,index) in pbanner" :key="index" slot="pagination"></div>
						</swiper>
					</div>
					...
					<div class='elastic-bottom' id='elastic-bottom'>
						<div class='elastic-select'>
							...
							<div class='select-count'>
								<div v-for='(item,index) in  countList ' class='select-count-btn' @click='tabCount(item.content,index,item.id)'>
						...
						<div class='elastic-select showTop'>
							...
							<div class='space'>
								...
								<span>平</span>
								<span class='space-tit'>(100平起购)</span>
							</div>
						</div>
						<div class='elastic-select showTop'>
	...
	export default {
		...
		methods: {
			...
			goPay() {
				...
				if (window.localStorage.getItem('wxBrowser') && (window.localStorage.getItem('token') == 'null')) {
					...
					window.localStorage.setItem('url', 'monthly')
				}
			...
			confirm() {
				...
				if (this.modelType == 1) { //调往提交订单页面
					...
					//  let url= this.$getOther.getUrl(window.location.href)
					...
					let info = {
						...
						weekCount: this.weekCount ? this.weekCount : 1,
						customizedInfo: this.customizedInfo,
						customizedMeal: this.customizedMeal,
						selectTime: this.selectTime,
					}
					this.area = 100
					this.$router.push({
			...
			getInfo() {
				...
				axios.post('/product/t-product-office/getProductOfficeVersion2', params).then(res => {
					...
					if (res.meta.success) {
						...
						// 微信浏览器				
						if (window.localStorage.getItem('wxBrowser') && (window.localStorage.getItem('evaluation') != null) && (window.localStorage
								.getItem('token') != null)) {
							this.wxEvaluation()
						}
			...
			calculation() {
				...
				let price1 = (((this.area * this.dailyPrice).toFixed(2) * this.areaRadixt + 0.00001).toFixed(2) * this.dailyNum +
					0.00001).toFixed(2)
				let price2 = (((this.area * this.depthPrice).toFixed(2) * this.areaRadixt + 0.00001).toFixed(2) * this.deepNum +
					0.00001).toFixed(2)
				// console.log(price2)
                // if(this.customizedId==3){
                //     	this.totalPrice = (((Number(price1) + Number(price2) + 0.0001).toFixed(2)) * 0.9   + 0.00001).toFixed(2) //双11季度9折
                // }else{
                    this.totalPrice =(Number(price1) + Number(price2) + 0.0001).toFixed(2)
                // }
		...
		watch: {
			...
			area(newval, oldval) {
				if (newval > 240) {
					this.modelType = 2
					this.msg = '您输入的面积符合我们的优享价，转到优享专区？'
					this.modelFlag = true
				}
				this.couponNum = 0
				this.couponId = ''
...
<style scoped>
  ...
  .elastic-bottom .elastic-select:nth-child(1){
	  /* margin-top: 3rem; */
  }
	...
	.elastic-select .select-count .select-count-btn {
		z-index: 1;
		height: 40px;
	}
