<template>
	<div>
		...
		<div class='content'>
			<div class='order showdaw-color'>
				...
				<div class='order-info' v-for='(item,index)  in list' :key='index'>
					...
					<div class='info-img'>
						<img :src="item.materPicUrl" mode=""></img>
					</div>
					<div class='info-new'>
						<div class='new-tit'>
		...
		<div class='order-bottom'>
			<div class='actual'>
				<span>实付款：</span>
				<span class='money'>￥{{info.payableMoney}}</span>
			...
			payMent() {
				...
				if (this.payType == 1) {
					if (!this.showWx) {
						...
						axios.post('/finance/payment/putPayShopOrderByWx', params).then(res => {
							if (res.meta.success) {
                         ...
                         params.append('orderId', this.id)
                         params.append('payableMoney', this.info.payableMoney)
                         axios.post('/finance/payment/putPayShopOrderByWxJs', params).then(res => {
                         	if (res.meta.success) {
				...
				if (this.payType == 2) {
					...
					params.append('orderId', this.id)
					params.append('payableMoney', this.info.payableMoney)
					axios.post('/finance/payment/putPayShopOrderByAli', params).then(res => {
						if (res.meta.success) {
