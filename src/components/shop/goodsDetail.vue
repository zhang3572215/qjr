<template>
	<div class='content'>
		...
		<div class='order-start' v-if='showContent' :class='showHead?"collage-head":""'>
			...
			<div class='start' v-if='info.orderStatus==7'>
				售后中
			</div>
		...
		<div>
			...
			<div class='meal' v-if='showContent'>
				<div class='meal-top' v-for='(item,index) in list' :key="index">
					<div class='meal-img'>
						<img :src='item.materPicUrl'></img>
					</div>
					<div>
						<div class='meal-info'>
							<div>
						...
						<div class='order-times'>
					   	...
					   	<span class='week  mr-right'>{{info.weeks}}</span> -->
							<span class='count mr-right'>{{item.className}}*{{item.propName}}<span></span></span>
			...
			<div class='order-info'>
				...
				<div class='start-date' v-if='info.orderStatus!=1'>付款时间：<span>{{info.payTime}}</span><span></span></div>
				<div class='pay-time' v-if='info.orderStatus!=1'>收货时间：<span>{{info.deliveTime}}</span><span></span></div>
			</div>
			...
			<div class='order-bottom' v-if='  info.orderStatus==1 || info.orderStatus==3 || info.orderStatus==4'>
				<!-- <div class='appointment' v-if='info.orderStatus==2'>预约</div> -->
				<div v-if='info.orderStatus==4' @click="toReviews">评价</div>
				<div v-if='info.orderStatus==3' class='complaint' @click='confirmReceipt(info.id)'>确认收货</div>
...
<script>
	...
	export default {
		...
		computed: {
			...mapGetters(['showCurrentListRefresh', 'showCleanFlag'])
		},
		methods: {
			...mapActions(['collageCltRefresh', 'collageCleanFlag']),
			toCustomer() {
				console.log(55444)
