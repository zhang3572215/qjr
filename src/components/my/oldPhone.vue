<template>
	<div>
		...
		<div class='content pad'>
			<div class='phone '>
				<div class='num'>旧手机号</div>
				<input type="number" v-model="phone" disabled="false" placeholder-style='color:#d9d9d9' placeholder="请输入旧手机号">
			</div>
			<div class='phone '>
				<div class='num'>验证码</div>
				<div class='code'>
					<input type="number" v-model="code" placeholder-style='color:#d9d9d9' placeholder="请输入手机验证码">
			...
			<div class='btn-bottom' @click="next">
				下一步
			</div>
		</div>
	...
	export default {
		...
		methods: {
			...
			next() {
				...
				axios.post('/sys/t-sys-app-user/verifyPhone', params).then(res => {
					this.isLoad = false
					if (res.meta.success) {
