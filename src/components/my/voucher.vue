<template>
	...
	<div  class='containerHeight' >
		<head-Piece :sign='showSign'>
			<template slot="content">我的抵用券</template>
		</head-Piece>
		...
		<div class='content-list'>
			<div class='list' v-for='(item,index) in list '>
				...
				<div class='list-right'>
					<div class='coupon-tit' v-if='item.productType==2'>办公室清洁专享</div>
					<div class='coupon-tit' v-if='item.productType==3'>家政清洁专享</div>
					<div class='coupon-tit' v-if='item.productType==4'>窗帘清洗专享</div>
					<div class='coupon-tit' v-if='item.productType==5'>地毯清洗专享</div>
					<div class='coupon-tit' v-if='item.productType==6'>酒店客房专享</div>
					<div class='coupon-tit' v-if='item.productType==7'>水晶灯清洗专享</div>
					<div class='coupon-tit' v-if='item.productType==8'>床垫清洗专享</div>
					<div class='coupon-tit' v-if='item.productType==9'>石材养护专享</div>
					<div class='coupon-tit' v-if='item.productType==9'>空调清洗专享</div>
					<div class='coupon-info'>满{{item.enoughMoneys}}可用</div>
					<div class='coupon-btn'>
						<div class='btn' @click="toMeal(item.itemType)">
							立即使用
						</div>
					</div>
					<div class='validity'>
...
<script>
	...
	export default {
		...
		created() {
			if(this.$getOther.getUrlInfo('token')){//微信扫码活动，活动有效，已领取->跳抵用金页(url上有token，你要先处理token)
				let token =this.$getOther.getUrlInfo('token')
			   window.localStorage.setItem('token', token)
			}
			if(this.$route.params.sign){//从活动礼包已经领去的页面，跳转过来！
				this.showSign=this.$route.params.sign//标记点击返回首页
			}
			this.getList()
...
<style scoped>
	.list {
		...
		/* 		background-image: url('../../static/my/voucher.png');
