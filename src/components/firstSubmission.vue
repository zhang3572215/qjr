	...
	<div class='conntent-info'>
		...
		<div v-show='collageAgr'>
			...
			<div class='order'>
				...
				<div class='order-area' @click="collageFocus">
					...
					<div >
						<van-field class='area space' type="digit" v-model="area" />
						<!-- <input class='area' type="number" v-model="area" @input="changeNumber($event)" /> -->
					</div>
				</div>
				<div class='order-date'  @click="triggerTime">
					<div class='date-tit'>
						预约服务时间
					</div>
				...
				<!-- <div class='order-date' @click="selectDate"  >
				<div class='date-tit'>
					开始服务日期
				</div>
				<div class='select-date'>
		...
		<div class='time-info' :class='collageTime?"showTime-info":""'>
			<div class='elastic-head'>
				<div class='time-list'>
					<div v-for='(item,index)  in dateList ' :key='index' v-if='index>1' :class=' collageDates==item.week ?"showTimeColor":""' @click="tabDate(item.week,item.date)">
		...
		<div class='shadowArea' v-if=" couponFlag==true  ||purpose==true  ||collageTime==true">
			...
			<showMoadea v-if='modelFlag' :cont='"您不是首次下单，不能参与活动！"' :showCancel='true'  :confirmspan='"查看订单"' @define='confirms' @remove='cancel'></showMoadea>
		   ...
		   resetHeight(){
			...
			calculation() {
				...
				if(this.area<=100){
					 ...
					 let area=this.area-100
								  this.totalPrice = Number(((this.price * area).toFixed(2) * this.areaRadixt).toFixed(2))+1
								  this.totalMoney = this.totalPrice
			...
			confirm(){
				...
				if (this.area < 100) {
					...
					axios.post('/office/t-office-order/createOfficeOrderByExperienceVersion2',
					params).then(res => {
						if(res.meta.success){
							console.log(res)
								this.isLoade=false
								// plus.storage.setItem('experience','1')
							let id={'id':res.data.orderId,type:'0'}
							...
							// this.$toast({
							//     message: '您不是首次下单，不能参与活动！', duration: 1500
							// })
			...
			confirms() {
				...
				this.modelFlag = false
				 ...
				 this.$router.push({'name':'orderFrequency',params:infoJson})
				// uni.navigateTo({
